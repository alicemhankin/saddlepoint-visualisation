...
distfunc = function(distrString){
  ...
  switch(distrString,
         ...
         mylist$ddist = function(x, var1, var2) dgamma(x, var1, var2);
         ...
         mylist$ddist = function(x, var1, var2) dchisq(x, var1);
         ...
         mylist$ddist = function(x, var1, var2) suppressWarnings(var1^x * exp(-var1) / factorial(x));
         mylist$rdist = function(n) runif(n);
         ...
         mylist$ddist = function(x, var1, var2) suppressWarnings(factorial(var2) * var1^x * (1-var1)^(var2-x)/(factorial(x)*factorial(var2-x)));
         mylist$rdist = function(n) runif(n);
         ...
         mylist$ddist = function(x, var1, var2) suppressWarnings(factorial(x+var2-1) * var1^var2 * (1-var1)^x / (factorial(x) * factorial(var2-1)));
         mylist$rdist = function(n) runif(n);
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    radioGroupButtons(
      ...
      choices = c("Choose K'(s)"="true", "Choose s"="false"),
      justified=TRUE
    ),
  ...
  dashboardBody(
    ...
    fluidRow(
      ...
      box(width=9,plotOutput("scaled_plot_old")),
    ...
    tags$div(style="margin-bottom:50px;",
             ...
             fluidRow(
               tabBox(width=9,id="tabs",
                      tabPanel("Saddlepoint Approximation", value="sadd", plotOutput("another_plot_old")),
                      tabPanel("Tilted Distribution",  value="tilt",plotOutput("yet_another_plot_old")),
               ...
               conditionalPanel(
                 ...
                 box(width=3, tags$div(class="sim", prettyCheckbox("togglesim", HTML("<b>Toggle 'Simulated' Line</b>"), FALSE)),
...
server <- function(input, output, session) {
  ...
  k_prime_inverse = reactive({Vectorize(function(x){s()[which.min(abs(x-cgfs_all_s()$k_prime))]},"x")})
  
  s = reactive({a = distlist()$max_s(input$var1, input$var2)
                a = ifelse(is.finite(a),a,0)
                seq(-10,a,by=0.05)})
  ...
  output$another_plot_old <- renderPlot({
    ...
    #true distribution
    if(input$toggletrue) lines(xvals(),  ifelse(xvals() > distlist()$min_x, ddist_x(), 0),
                               col=cols[3], lty=lty[1], lwd=lwd[1])
    
    #saddlepoint approximation
    if(input$togglesadl) lines(cgfs_all_s()$k_prime, exp(cgfs_all_s()$k - s() * cgfs_all_s()$k_prime) / 
    ...
    tryCatch(if (input$togglesim){
      if(distlist()$discrete){
        lines(table(scaled_x())/length(scaled_x()),lwd=lwd[3], col=cols[1], type='l', lty=lty[3])
      }else{
        lines(density(scaled_x(), adjust=0.8), lwd=lwd[3], col=cols[1], lty=lty[3])
      }
    }, error=function(e){})
  ...
  output$yet_another_plot_old <- renderPlot({
    ...
    cols = c("white",	"#E69F00", "#0072B2","#a5becc")
    log = ifelse(input$togglelog == TRUE, "y", "")
    lty=if(input$togglecol) c("dotted", "solid") else rep("solid", 2)
    ...
    if(input$colourswitch=="true"){
      ...
      if(input$toggletilt_move) lines(xvals(), 
                                      ifelse(xvals() > distlist()$min_x, exp(k_prime_inverse()(input$x) * xvals() - cgfs()(k_prime_inverse()(input$x))$k) *
                                               ddist_x(), 0),
      ...
      if(input$toggletilt_move) lines(xvals(), 
                                      ifelse(xvals() > distlist()$min_x, exp(input$s * xvals() - cgfs_input_s()$k) * ddist_x(), 0),
                                      col=cols[2], lty=lty[1], lwd=lwd[1])
    }
    ...
    legend(x="topright", legend = c("Tilted distribution",
                                    "Normal Approximation"), col=cols[2:3], lwd=4, lty=lty)
  ...
  output$xLoopSlider <- renderUI({
    ...
    max = min(distlist()$max_s(input$var1, input$var2), max)
    max = round(cgfs()(max)$k_prime,2)
    min = max(distlist()$min_x, -10, round(cgfs()(-10)$k_prime,2))
  ...
  output$sLoopSlider <- renderUI({
    ...
    min = round(ifelse(typeof(k_prime_inverse()(min))=="list", 0, k_prime_inverse()(min)),2)
    max  = ifelse(typeof(k_prime_inverse()(10))=="list", 1, k_prime_inverse()(10))
    max = round(min(distlist()$max_s(input$var1, input$var2), max),2)
  ...
  output$first_slider <- renderUI({
    sliderInput("var1", label= "", step = distlist()$step[1],
  ...
  output$second_slider <- renderUI({
    conditionalPanel(condition = distlist()$condition,
                     sliderInput("var2", label= "", step=distlist()$step[2],
